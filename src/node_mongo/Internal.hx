...
@:tink class CollectionBase<T> {
  
  //function key(o:Dynamic) {
    //return switch Type.typeof(o) {
      //case TNull | TInt | TFloat | TBool | TClass(Date): Std.string(o);
      //case TObject:
        //var keys = Reflect.fields(o);
        //keys.sort(
      //case TClass(String): o;
      //case TClass(Array): o.map(key).join(',');
      //default: throw 'unhashable';
    //}
  //}
  ...
  function _findAndModify<A>(proto:A, match:Dynamic, update:Dynamic, options:Dynamic):Unsafe<A>
    return 
      native >> @f(collection) 
        Node.await(collection.findAndModify(match, [], update, options)).map(function (x) return x.map(function (x) return x));
}
...
abstract DbParams(DbParamObj) from DbParamObj to DbParamObj {
  ...
  @:from static public function ofString(s:String):DbParams {
    var ret:DbParamObj = 
      if (wlogin.match(s)) {
...
@:tink class DbBase {
  ...
  public function collection<A>(name:String):Collection<A> {
