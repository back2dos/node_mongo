...
private class Parser {
  ...
  static function field(e:Expr) 
    return
      switch e {
        case macro $path++: make(path, Inc(macro 1));
        case macro $path--: make(path, Inc(macro -1));
        case macro !$path: make(path, Unset);
        case macro $path = $v: make(path, Set(v));
        case { expr: EBinop(OpAssignOp(op), path, v) }:
          switch (op) {
        ...
        case macro if ($condition) $update:
          var ret = field(update);
